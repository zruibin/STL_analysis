...
multiset的特性及其用法和set完全相同，唯一的区别就是multiset允许键值key重复，因此multiset的插入操作采用的是底层RB-Tree的`insert_equal()`而非`insert_unique()`，有关set容器的介绍前往博文[《STL源码剖析——关联容器之set》](http://blog.csdn.net/chenhanzhun/article/details/39525489)。本文的源码出自SGI STL中的`<stl_multiset.h>`文件。
