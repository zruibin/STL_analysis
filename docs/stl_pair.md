...
pair在关联容器中经常被使用，它提供了两个成员变量first和second，由于pair是一个struct，所以其成员变量的属性是public。在pair struct中提供了构造函数和拷贝构造函数，同时提供了两个最基本的操作`operator==`和`operator<`重载，其他的操作符重载都是基于前面两种的变形。本文源码来自SGI STL中的`<stl_pair.h>`文件。
...
* 《STL源码剖析》侯捷
* [《STL源码剖析-- stl_pair.h》](http://blog.csdn.net/mdl13412/article/details/6643400)
