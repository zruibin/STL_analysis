## 前言
在STL中，算法是经常被使用的，算法在整个STL中起到非常重要的作用。本节介绍的是一些基本算法，包含`equal，fill，fill_n，iter_swap，lexicographical_compare，max，min，mismatch，swap，copy，copy_backward，copy_n`。其中一个比较重要的算法就是copy，针对copy的剖析在源码中可以看到详细的注解。本文剖析的源码出自SGL STL中的`<stl_algobase.h>`文件。
