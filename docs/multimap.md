...
multimap的特性及其用法和map完全相同，唯一的区别就是multimap允许键值key重复，因此multimap的插入操作采用的是底层RB-Tree的`insert_equal()`而非`insert_unique()`，有关map容器的介绍前往博文[《关联容器之map》](http://blog.csdn.net/chenhanzhun/article/details/39529425)。本文的源码出自SGI STL中的`<stl_multimap.h>`文件。
